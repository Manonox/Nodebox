...
public class Add : Node
{
	...
	public override Node Polymorph(PinWireChange change) =>
        PolymorphHelpers.ToConnectedTypeIfRegistered(typeof(Add<>), change);
}
...
[Polymorphic(typeof(Add))]
public class Add<T> : Add
{
    ...
    public override void Evaluate() {
        ...
        if (t == typeof(string)) { SetOutput(0, GetInput<string>(0) + GetInput<string>(1)); return; }
        if (t == typeof(Angles)) { SetOutput(0, GetInput<Angles>(0) + GetInput<Angles>(1)); return; }
        if (t == typeof(Color)) { SetOutput(0, GetInput<Color>(0) + GetInput<Color>(1)); return; }
    }

    public override Node Polymorph(PinWireChange change) =>
        PolymorphHelpers.ToNonGenericIfAllDisconnected<Add>(change);
