...
public class Multiply : Node
{
	...
	public override Node Polymorph(PinWireChange change) =>
        PolymorphHelpers.ToConnectedTypeIfRegistered(typeof(Multiply<>), change);
}
...
[Polymorphic(typeof(Multiply))]
public class Multiply<T> : Multiply
{
    ...
    public override void Evaluate() {
        ...
        if (t == typeof(Vector4)) { SetOutput(0, GetInput<Vector4>(0) * GetInput<Vector4>(1)); return; }
        ...
        if (t == typeof(Angles)) { SetOutput(0, GetInput<Angles>(0).Multiply(GetInput<Angles>(1))); return; }
        if (t == typeof(Rotation)) { SetOutput(0, GetInput<Rotation>(0) * GetInput<Rotation>(1)); return; }
        if (t == typeof(Color)) { SetOutput(0, GetInput<Color>(0) + GetInput<Color>(1)); return; }
    ...
    public override Node Polymorph(PinWireChange change) =>
        PolymorphHelpers.ToNonGenericIfAllDisconnected<Multiply>(change);
}
