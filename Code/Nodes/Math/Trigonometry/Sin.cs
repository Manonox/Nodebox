...
public class Sin<T> : Node
{
    ...
    public override void Evaluate() {
        if (typeof(T) == typeof(float)) {
            ...
            SetOutput(1, MathF.Sin(MathX.DegreeToRadian(GetInput<float>(0))));
            return;
        }

        if (typeof(T) == typeof(double)) {
            SetOutput(0, Math.Sin(GetInput<double>(0)));
            SetOutput(1, Math.Sin(GetInput<double>(0) * Math.PI / 180.0));
            return;
        }
